...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className="mx-auto max--[1600px] px-6 lg:px-12 py-4 flex items-center justify-between gap-6">
        <button onClick={() => scrollTo("home")} className="flex flex-col items-start group">
          <div className="font-display italic text-2xl tracking-wider leading-none" style={{transform: "skewX(-10deg"}}>
            ...
            <span className="texxt-electric">PUTRA</span>
          </div>
          <div className="font-maker text-[11px] text-electric tracking-widest">
            {"{ DEV PORTFOLIO }"}
          </div>
        </button>
        ...
        className="font-display italic tracking-widest text-sm px-6 py-2.5 bg-none text-ink ribbon-clip hover:bg-electric transition-colors"
        style={{transform: "skewX(-10deg"}}>
          <span style={{transform: "skewX(10deg)"}} className="inline-block">LETS TALK</span>
        </button>
      </div>
    </motion.header>
  )
